...
function StatusBadge({ status }: { status: ClaimStatus }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border shadow-sm ring-1 ring-white/70 ${cfg.color} ${cfg.bg} ${cfg.border}`}>
...
function RoleBadge({ role }: { role: Position }) {
  ...
  const Icon = cfg.icon;
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border border-white/70 shadow-sm ${cfg.color} ${cfg.bg}`}>
...
function RejectModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm px-4">
      <div className="bg-white/95 rounded-3xl shadow-2xl border border-white/80 w-full max-w-md p-6 space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-red-100 to-rose-200 border border-red-200 flex items-center justify-center">
        ...
        <textarea
          ...
          rows={4}
          className="w-full text-sm px-4 py-3 border border-rose-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-400 resize-none bg-rose-50/70 placeholder-rose-300 text-gray-700"
          autoFocus
        />
        <div className="flex gap-3">
          <button
            onClick={onCancel}
            className="flex-1 py-2.5 text-sm font-semibold text-gray-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 rounded-xl shadow-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Confirm Rejection
          </button>
        </div>
...
function VerifyModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm px-4">
      <div className="bg-white/95 rounded-3xl shadow-2xl border border-white/80 w-full max-w-md p-6 space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-green-100 to-emerald-200 border border-green-200 flex items-center justify-center">
        ...
        <div className="bg-slate-50/90 rounded-2xl p-3 text-xs text-gray-600 space-y-1 border border-slate-200">
        ...
        <textarea
          ...
          rows={3}
          className="w-full text-sm px-4 py-3 border border-green-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-400 resize-none bg-green-50/60 placeholder-gray-400 text-gray-700"
        />
        <div className="flex gap-3">
          <button
            onClick={onCancel}
          ...
          <button
            ...
            className="flex-1 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 rounded-xl shadow-sm transition-colors flex items-center justify-center gap-2"
          >
...
function ClaimCard({
  ...
  return (
    <div className={`bg-gradient-to-br from-white via-white to-slate-50 border rounded-3xl overflow-hidden transition-all h-full ${loading ? 'opacity-60 pointer-events-none' : 'hover:border-slate-300 hover:shadow-lg hover:-translate-y-0.5'} border-slate-200 shadow-sm`}>
      <div className="p-4 sm:p-5">
        <div className="flex items-start gap-3">
          {/* Avatar */}
          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ring-2 ring-white shadow-sm ${ROLE_CONFIG[claim.claimantPosition].bg} ${ROLE_CONFIG[claim.claimantPosition].color}`}>
          ...
          <div className="flex flex-col items-start sm:items-end gap-2 shrink-0">
            <p className="text-base font-extrabold text-slate-900">₹{claim.amount.toLocaleString()}</p>
            <StatusBadge status={claim.status} />
          </div>
        </div>

        {/* Description */}
        <p className="text-xs text-slate-600 mt-3 line-clamp-2 leading-relaxed bg-slate-50/80 border border-slate-200 rounded-xl p-2.5">
        ...
        {(claim.transactionId || hasAttachments) && (
          <div className="mt-2 flex flex-wrap items-center gap-2">
            {claim.transactionId && (
              <span className="inline-flex items-center px-2 py-1 rounded-lg text-[11px] font-mono font-semibold text-blue-700 bg-blue-50 border border-blue-200 shadow-sm">
            ...
            {hasAttachments && claim.attachments.map((att, i) => (
              <a
                ...
                className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 shadow-sm hover:bg-emerald-100 transition-colors"
              >
                <FileText size={11} />
        ...
        {claim.verifications && claim.verifications.length > 0 && (
          <div className="mt-3 space-y-1.5 bg-emerald-50/80 rounded-2xl p-2.5 border border-emerald-200">
        ...
        {claim.status === 'REJECTED' && (
          <div className="mt-2 bg-rose-50 rounded-2xl p-2.5 border border-rose-200 space-y-0.5">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto sm:justify-end">
            {canReject && (
              <button
                ...
                className="flex-1 sm:flex-none justify-center flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-red-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-xl shadow-sm transition-colors"
              >
            ...
            {canVerify && (
              <button
                ...
                className="flex-1 sm:flex-none justify-center flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-green-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-xl shadow-sm transition-colors"
              >
...
export function VerifyReimbursementView({
  ...
  return (
    <div className="space-y-6 lg:space-y-7">
      ...
      {detailsTarget && (
        <div className="hidden lg:flex fixed inset-0 z-40 items-center justify-center bg-slate-900/60 backdrop-blur-sm px-4">
          <div className="w-full max-w-2xl rounded-3xl bg-white/95 shadow-2xl border border-white/80 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-emerald-50">
              <div>
              ...
              <button
                ...
                className="p-1.5 rounded-xl text-slate-500 hover:bg-slate-200 hover:text-slate-700 transition-colors"
              >
                <XCircle size={18} />
              </button>
            </div>
      ...
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 rounded-3xl border border-slate-200 bg-gradient-to-r from-white via-slate-50 to-emerald-50/70 p-4 shadow-sm">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Verify Reimbursements</h1>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full lg:w-auto">
          {userFests.length > 1 && (
            <div className="relative w-full sm:w-auto">
              <button
                ...
                className="w-full sm:w-auto flex items-center justify-between sm:justify-start gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 hover:border-slate-300 shadow-sm transition-colors max-w-xs sm:max-w-none"
              >
              ...
              {festDropdownOpen && (
                <div className="absolute left-0 top-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl z-10 min-w-[220px] w-full sm:w-auto overflow-hidden">
                  {userFests.map((f, idx) => (
                    <button
                      key={f._id}
          ...
          <button
            onClick={fetchClaims}
            disabled={loading}
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 hover:border-slate-300 shadow-sm transition-colors disabled:opacity-60"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          { label: 'Pending', value: pendingCount, color: 'text-orange-700', bg: 'bg-gradient-to-br from-orange-50 to-amber-50', border: 'border-orange-200' },
          { label: 'Verified', value: verifiedCount, color: 'text-emerald-700', bg: 'bg-gradient-to-br from-emerald-50 to-green-50', border: 'border-emerald-200' },
          { label: 'Total ₹', value: `₹${totalAmount.toLocaleString()}`, color: 'text-slate-700', bg: 'bg-gradient-to-br from-slate-50 to-gray-50', border: 'border-slate-200' },
        ].map(s => (
          <div key={s.label} className={`${s.bg} border ${s.border} rounded-2xl p-3 text-center shadow-sm`}>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-4 space-y-4 shadow-sm">
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 bg-slate-50/40" 
          />
        </div>
            ...
            {currentUserPosition === 'FEST_COORDINATOR' && availableRoleFilters.map(r => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors border ${
                  filterRole === r ? 'bg-slate-900 text-white border-slate-900 shadow-sm' : 'bg-slate-50 text-slate-600 border-slate-200 hover:border-slate-300'
                }`}
              >
            ...
            {(['date', 'amount', 'role'] as const).map(s => (
              <button
                ...
                className={`ml-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-colors border ${
                  sortBy === s ? 'bg-emerald-600 text-white border-emerald-600 shadow-sm' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100'
                }`}
              >
              ...
              <table className="w-full min-w-[920px]">
                ...
                <tbody className="divide-y divide-slate-100">
                  {displayed.map(claim => {
                    ...
                    return (
                      <tr key={claim._id} className="hover:bg-emerald-50/40 transition-colors">
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-2">
                            {claim.transactionId && (
                              <button
                                ...
                                className="inline-flex items-center px-2 py-1 rounded-lg text-[11px] font-mono font-semibold text-blue-700 bg-blue-50 border border-blue-200 shadow-sm hover:bg-blue-100 transition-colors"
                              >
                                TXN: {claim.transactionId}
                              </button>
                            )}
                          ...
                          <div className="flex items-center gap-2">
                            {canReject && (
                              <button
                                ...
                                className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-red-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-xl shadow-sm transition-colors disabled:opacity-60"
                              >
                            ...
                            {canVerify && (
                              <button
                                ...
                                disabled={actionLoading === claim._id}
                                className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-xl shadow-sm transition-colors disabled:opacity-60"
                              >
